...
function addDislike($sender, $reciever, $mysqli) {
    $stmt = $mysqli->prepare("INSERT INTO likes (sender, reciever, type) VALUES (?, ?, 'd')");
    $stmt->bind_param("ss", $sender, $reciever);
    $stmt->execute();
    $stmt->close();
}
...
function addLike($vid, $sender, $reciever, $conn) {
...
function getLikes($id, $mysqli) {
    $stmt = $mysqli->prepare("SELECT * FROM likes WHERE reciever = ? AND type = 'l'");
    $stmt->bind_param("s", $id);
    $stmt->execute();
    $result = $stmt->get_result();
...
function getDislikes($id, $mysqli) {
    $stmt = $mysqli->prepare("SELECT * FROM likes WHERE reciever = ? AND type = 'd'");
    $stmt->bind_param("s", $id);
    $stmt->execute();
    $result = $stmt->get_result();
